...
)
{
	...
	switch(prop->definition.type)
	{
		case STDIF_CONFIGURABLE_TYPE_DOUBLE:
			...
			if((value_d > INT16_MAX) || (value_d < INT16_MIN))
				return EXTIT_STATUS_INVALID;

			*valuep = (int16_t) value_d;
			break;

		case STDIF_CONFIGURABLE_TYPE_FLOAT:
			value_f = truncf(*((float *) base));

			if((value_f > INT16_MAX) || (value_f < INT16_MIN))
				return EXTIT_STATUS_INVALID;

			*valuep = (int16_t) value_f;
			break;
		...
		case STDIF_CONFIGURABLE_TYPE_INT32:
			...
			if((value_i32 > INT16_MAX) || (value_i32 < INT16_MIN))
				return EXTIT_STATUS_INVALID;

			*valuep = (int16_t) value_i32;
			break;

		case STDIF_CONFIGURABLE_TYPE_INT64:
			value_i64 = *((int64_t *) base);

			if((value_i64 > INT16_MAX) || (value_i64 < INT16_MIN))
				return EXTIT_STATUS_INVALID;

			*valuep = (int16_t) value_i64;
			break;
		...
		case STDIF_CONFIGURABLE_TYPE_UINT16:
			...
			if(value_u16 > (uint16_t) INT16_MAX)
				return EXTIT_STATUS_INVALID;

			*valuep = (int16_t) value_u16;
			break;
		...
		case STDIF_CONFIGURABLE_TYPE_ENUM32:
			...
			if(value_u32 > (uint32_t) INT16_MAX)
				return EXTIT_STATUS_INVALID;

			*valuep = (int16_t) value_u32;
			break;

		case STDIF_CONFIGURABLE_TYPE_UINT64:
			value_u64 = *((uint64_t *) base);

			if(value_u64 > (uint64_t) INT16_MAX)
				return EXTIT_STATUS_INVALID;

			*valuep = (int16_t) value_u64;
			break;
